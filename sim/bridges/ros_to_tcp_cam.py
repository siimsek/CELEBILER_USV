...
JPEG_QUALITY = int(os.environ.get("SIM_CAM_JPEG_QUALITY", "60"))
# Shorter interval = lower viewer latency (bounded by ROS image rate); raises TCP CPU slightly.
STREAM_INTERVAL_S = max(0.01, float(os.environ.get("SIM_CAM_STREAM_DT", "0.02")))
# Eski davranış: depth=50 → ROS kuyruğunda onlarca eski kare; cam TCP tarafında "aşırı geriden" algı.
# depth=1 + BEST_EFFORT: Gazebo kamera (sensor_data) ile uyumlu, her zaman en güncel kare.
_CAM_SUB_QOS = QoSProfile(
...
class CameraBridge(Node):
    ...
    def image_callback(self, msg):
        try:
            ...
            # Compress to JPG
            ret, buf = cv2.imencode('.jpg', cv_img, [int(cv2.IMWRITE_JPEG_QUALITY), JPEG_QUALITY])
            if ret:
    ...
    def handle_client(self, client_sock):
        try:
            ...
            while True:
                ...
                seq = self.latest_seq if self.latest_jpeg is not None else -1
                if frame and seq != last_sent_seq:
